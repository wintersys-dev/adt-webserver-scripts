...
then
        ...
        then
                ...
                ${HOME}/installation/InstallMergerFS.sh ${BUILDOS}
        fi
		if ( [ ! -f ${HOME}/runtime/mergerfs.config ] )
		then
			/bin/cp ${HOME}/services/datastore/assets/config/mergerfs.conf ${HOME}/runtime/mergerfs.config
		fi
...
do
	...
	${HOME}/services/datastore/operations/MountDatastore.sh "asset" "local" "${directory}"
...
do
        ...
        then				
                ...
                if ( [ "${count}" = "10" ] )
                then
                        ${HOME}/services/email/SendEmail.sh "DIRECTORY ${asset_directory} NOT MOUNTED" "A mount has failed for directory ${asset_directory}" "ERROR"
                else
