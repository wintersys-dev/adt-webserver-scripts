...
${HOME}/services/datastore/operations/GetFromDatastore.sh "backup" "applicationsourcecode.tar.gz" "." "${period}"
...
then
	...
	${HOME}/services/email/SendEmail.sh "Potential missing webroot backup for periodicity ${BUILD_ARCHIVE_CHOICE} in your datastore" "A Backup that I expected seems to be missing in the git repository" "ERROR"
else
	/bin/echo "Backup verfied as consistent"
	${HOME}/services/email/SendEmail.sh "Backup has been made to your datastore" "A Backup has been successfully written to your datastore" "INFO"
fi
