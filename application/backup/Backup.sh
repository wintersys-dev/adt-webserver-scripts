...
then
        directories_to_omit="`/bin/grep "^WEBROOT_DIRECTORIES_TO_MOUNT:" ${HOME}/runtime/application.dat | /bin/sed 's/WEBROOT_DIRECTORIES_TO_MOUNT://g' | /bin/sed 's/:/ /g'`"

        if ( [ "${directories_to_omit}" != "" ] )
        then
                for dir in ${directories_to_omit}
                do
...
${HOME}/application/backup/VerifyBackupPresent.sh ${period}
...
then
        ${HOME}/services/datastore/filesystems-sync/heavyweight/DeleteHistoricalAdditions.sh "/var/www/html"
fi
