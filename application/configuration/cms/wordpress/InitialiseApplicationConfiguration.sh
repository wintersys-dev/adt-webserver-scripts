...
if ( [ -L ${webroot_directory}/wp-config.php ] )
then
...
then
        ...
        then
                ...
                then
                        ...
                        #The special plugin to hide the admin login area with the value set in the application configuration file
        ...
        /bin/grep "table_prefix" ${webroot_directory}/wp-config.php | /usr/bin/awk -F"\'" '{print $2}'  > /var/www/html/dbp.dat
        /bin/chown www-data:www-data /var/www/html/dbp.dat
        fi
else
