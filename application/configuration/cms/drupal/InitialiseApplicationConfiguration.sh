...
if ( [ -L ${webroot_directory}/${webroot_subdirectory}/sites/default/settings.php ] )
then
...
user_tls=""
database_credentials="${HOME}/application/configuration/cms/drupal/database_credentials_mysql.dat"
...
/bin/cp ${database_credentials_file} ${HOME}/runtime/database_credentials.dat
...
then
        ...
        /bin/chmod -R 750 ${webroot_directory}/${webroot_subdirectory}/sites/default/files
fi

private_ip="`${HOME}/utilities/processing/GetIP.sh`"
BUILD_MACHINE_IP="`${HOME}/utilities/config/ExtractConfigValue.sh 'BUILDMACHINEIP'`"
/bin/sed -i "s/XXXXBUILD_MACHINE_IPXXXX/${BUILD_MACHINE_IP}/" ${webroot_directory}/${webroot_subdirectory}/sites/default/settings.php
/bin/sed -i "s/XXXXPRIVATE_IPXXXX/${private_ip}/" ${webroot_directory}/${webroot_subdirectory}/sites/default/settings.php
...
then
        ...
        /bin/grep "prefix" ${webroot_directory}/sites/default/settings.php | /usr/bin/awk -F"'" '{print $4}' > /var/www/html/dbp.dat
        /bin/chown www-data:www-data /var/www/html/dbp.dat
else    
      #  if ( [ -f ${webroot_directory}/${webroot_subdirectory}/sites/default/default.settings.php ] )
      #  then
