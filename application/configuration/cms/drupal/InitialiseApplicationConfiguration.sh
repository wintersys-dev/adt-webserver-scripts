#!/bin/sh
###########################################################################################################
# Description:This script will generate a ${webroot_directory}/web/sites/default/settings.php using the values 
...
then
        ...
        then
                ...
                then
                        ...
                        /usr/sbin/drush si --no-interaction --db-url="${driver}://${username}:${password}@${HOST}:${DB_PORT}/${database}?module=${driver}#${dbprefix} --db-prefix=${dbprefix} --extra=--ssl"
                        /usr/sbin/drush cr
