...
do
	...
	then
		if ( [ "${BUILDOS}" = "ubuntu" ] )
		then
			eval ${install_command} gawk ${tail_options}
		fi
...
then
	${HOME}/services/email/SendEmail.sh "INSTALLATION ERROR GAWK" "I believe that gawk hasn't installed correctly, please investigate" "ERROR"
else
