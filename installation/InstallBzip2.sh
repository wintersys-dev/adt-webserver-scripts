#!/bin/sh
######################################################################################################
# Description: This script will install bzip2
# Author: Peter Winter
# Date: 17/01/2017
#######################################################################################################
# License Agreement:
...
do
	...
	then
		if ( [ "${BUILDOS}" = "ubuntu" ] )
		then
			eval ${install_command} bzip2 ${tail_options}
		fi
...
if ( [ ! -x /usr/bin/bzip2 ] && [ "${count}" = "5" ] )
then
	${HOME}/services/email/SendEmail.sh "INSTALLATION ERROR Bzip2" "I believe that bzip2 hasn't installed correctly, please investigate" "ERROR"
else
	/bin/touch ${HOME}/runtime/installedsoftware/InstallBzip2.sh	
fi
