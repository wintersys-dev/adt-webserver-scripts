#!/bin/sh
######################################################################################################
# Description: This script will install QR Encode
# Author: Peter Winter
# Date: 17/01/2017
#######################################################################################################
# License Agreement:
...
count="0"
while ( [ ! -f /usr/bin/qrencode ] && [ "${count}" -lt "5" ] )
do
	if ( [ "${manager}" != "" ] )
	then
		if ( [ "${BUILDOS}" = "ubuntu" ] )
		then
			eval ${install_command} qrencode ${tail_options}
		fi
...
if ( [ ! -x /usr/bin/qrencode ] && [ "${count}" = "5" ] )
then
	${HOME}/services/email/SendEmail.sh "INSTALLATION ERROR QR Encode" "I believe that QR Encode hasn't installed correctly, please investigate" "ERROR"
else
	/bin/touch ${HOME}/runtime/installedsoftware/InstallQREncode.sh	
fi
