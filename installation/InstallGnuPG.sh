...
do
	...
	then
		if ( [ "${BUILDOS}" = "ubuntu" ] )
		then
    			eval ${install_command} gnupg ${tail_options}
		fi
...
then
	${HOME}/services/email/SendEmail.sh "INSTALLATION ERROR Gnupg" "I believe that gnupg hasn't installed correctly, please investigate" "ERROR"
else
