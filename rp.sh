...
then
	...
	if ( [ "`${HOME}/services/datastore/operations/ListFromDatastore.sh "wire-guard-emails" "SEND_NOTIFICATION_EMAIL"`" = "" ] )
	then
        ${HOME}/services/datastore/operations/DeleteFromDatastore.sh "wire-guard-emails" "SEND_NOTIFICATION_EMAIL" "local"
	fi
fi
...
if ( [ "${count}" = "5" ] )
then
	${HOME}/services/email/SendEmail.sh "SSL CERFICICATES NOT SUCCESSFULLY INSTALLED" "The ssl certificates for a reverse proxy have not been successfully installed" "ERROR"
fi

/bin/echo "${0} Sending 'successful build' notification email"
${HOME}/services/email/SendEmail.sh "A REVERSE PROXY HAS BEEN SUCCESSFULLY BUILT" "A reverse proxy has been successfully built and primed as is rebooting ready for use" "INFO"
