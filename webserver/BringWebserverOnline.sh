...
# Author : Peter Winter
# Date   : 13/07/2016
# Description : This script will take the webserver offline and will prevent it from restarting
# until BringWebserverOnline.sh is run
########################################################################################
# License Agreement:
...
then
  ...
  /bin/sed -i 's/XXXX_BORKED_XXXX//g' /etc/nginx/sites-available/${WEBSITE_NAME}
...
then
	${HOME}/utilities/processing/RunServiceCommand.sh php${PHP_VERSION}-fpm stop
	${HOME}/utilities/processing/RunServiceCommand.sh php${PHP_VERSION}-fpm start
  /bin/sed -i 's/XXXX_BORKED_XXXX//g' /etc/apache2/sites-available/${WEBSITE_NAME}
...
then
	${HOME}/utilities/processing/RunServiceCommand.sh php${PHP_VERSION}-fpm stop
	${HOME}/utilities/processing/RunServiceCommand.sh php${PHP_VERSION}-fpm start
  /bin/sed -i 's/XXXX_BORKED_XXXX//g' /etc/lighttpd/lighttpd.conf
