...
webserver_ips="`${HOME}/services/datastore/config/wrapper/ListFromDatastore.sh "config" "webserverips/*"`"
updated="0"
...
do
        ...
        then
                ...
                then
                        if ( [ "`${HOME}/services/datastore/config/wrapper/ListFromDatastore.sh "config" "beingbuiltips/${webserver_ip}"`" = "" ] )
                        then
