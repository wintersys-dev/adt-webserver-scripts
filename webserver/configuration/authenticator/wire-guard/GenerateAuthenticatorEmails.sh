...
do
        ...
        do
               ...
               # /bin/echo "${email_address}:${client_url}" >> ${HOME}/runtime/wire-guard/clientconfig
                ...
                then
                        message="<!DOCTYPE html> <html> <body> <h1>Wireguard authorisation for ${WEBSITE_URL_ORIGINAL}</h1> <p>Click the below link in order to authorise your wireguard access for ${WEBSITE_URL_ORIGINAL} </p> <a href='"${qrcode_url}"'>View Your Wireguard QR Code</a> <br> <a href='"${client_url}"'>View Your Wireguard QR Client File</a>  </br> <br> <a href='"${backup_qrcode_url}"'>View Your Backup Wireguard QR Code</a> </br> <br> <a href='"${backup_client_url}"'>View Your Backup Wireguard QR Client File</a>  </br>  <br></br> For future resiliance, install your primary and your backup QR codes now into your wireguard app. </body> </html>"
                else
