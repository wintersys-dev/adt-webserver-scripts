...
if ( [ ! -d ${HOME}/logs/configure_nginx ] )
then
        /bin/mkdir -p ${HOME}/logs/configure_nginx
fi

log_file="nginx_out_`/bin/date | /bin/sed 's/ //g'`"
err_file="nginx_err_`/bin/date | /bin/sed 's/ //g'`"

/bin/echo "Log file is at: ${HOME}/logs/configure_nginx/${log_file}"
/bin/echo "Error file is at: ${HOME}/logs/configure_nginx/${err_file}"

exec 1>>${HOME}/logs/configure_nginx/${log_file}
exec 2>>${HOME}/logs/configure_nginx/${err_file}
...
${HOME}/services/email/SendEmail.sh "THE NGINX WEBSERVER HAS BEEN INSTALLED" "Nginx webserver is installed and primed" "INFO"
