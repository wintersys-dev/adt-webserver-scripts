...
if ( [ "${sql_command}" != "" ]  )
then
        ${mysql} --defaults-extra-file=${credentials_file} --ssl-mode=REQUIRED --silent --raw ${verify_cert} -A ${DB_N} -e "${sql_command}"
else
        ${mysql} --defaults-extra-file=${credentials_file} --ssl-mode=REQUIRED  ${verify_cert} -A ${DB_N}
fi
