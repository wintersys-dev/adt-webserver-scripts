...
do
        ...
        then
                ...
                ${HOME}/services/email/SendEmail.sh "SSH CONNECTION FROM A NEW IP ADDRESS" "There has been a new connection from an unknown IP ${ssh_client_ip} to machine `/usr/bin/hostname`" "ERROR"
        fi
done
